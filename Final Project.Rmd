...
joined_frames %>% 
  ...
  ggplot(aes(x = FY, y = SALES_TOTAL, fill = CENTER, color = CENTER)) +
...
```{r}
# Tracking Savings per fiscal year
# saving_noNA <- joined_frames 
# saving_noNA <- saving_noNA %>% 
#   replace(is.na('PSAVED'), 0) %>% 
#   replace(is.na('SSAVED'), 0) %>% 
#   replace(is.na('TSAVED'), 0) %>% 
#   replace(is.na('QSAVED'), 0)
# # saving_noNA <- replace(saving_noNA, is.na('SSAVED'), 0)
# # saving_noNA <- replace(saving_noNA, is.na('TSAVED'), 0)
# # saving_noNA <- replace(saving_noNA, is.na('QSAVED'), 0)
# saving_noNA
...
joined_frames %>% 
  ...
  ggplot(aes(x = FY, y = TOTAL_SAVED, color = CENTER)) +
  geom_bar(stat = "identity", size = 2) +
  labs(title = "Total Saved",
       x = "Fiscal Year",
...
graphed <- joined_frames %>% 
  select(US_TITLE, ARC2, PSAVED, SSAVED, TSAVED, QSAVED) %>% 
  filter(!is.na(US_TITLE)) %>%
  group_by(US_TITLE, ARC2) %>%
  mutate(TOTAL_SAVED = sum(PSAVED, SSAVED, TSAVED, QSAVED, na.rm = TRUE)) %>%
  arrange(TOTAL_SAVED) %>%
  slice_head(n=3)%>%
  ggplot(aes(x = US_TITLE, y = TOTAL_SAVED)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  labs(title = "Top 10 Recommendations by Savings Across Top 5 Industries",
